...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local").setAppName("WikipediaRankings")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.filter(article => article.mentionsLanguage(lang))
       .aggregate(0)((acc,_) => acc+1 ,(acc1,acc2)=>acc1+acc2)
...
//  }.groupByKey()
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap(article => langs.map(lang =>
      (lang,article.mentionsLanguage(lang) match {case true => 1 case false => 0}))).filter(_._2!= 0)
  ...
  def main(args: Array[String]): Unit = {
    ...
    //val temp: Int = occurrencesOfLang("Java", wikiRdd)
    println(langsRanked)
